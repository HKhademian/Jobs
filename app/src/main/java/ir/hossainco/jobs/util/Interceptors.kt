...
object Interceptors {
	const val CACHE_TIME_ZERO = 0 /* s */
	const val CACHE_TIME_NANO = 10 /* s */
	const val CACHE_TIME_TINY = 20 /* s */
	const val CACHE_TIME_MINY = 30 /* s */
	const val CACHE_TIME_NORM = 60 /* s */
	const val CACHE_TIME_HOUR = 60 * 60 /* s */
	const val CACHE_TIME_ADAY = 60 * 60 * 24 /* s */
	const val CACHE_TIME_WEEK = 60 * 60 * 24 * 7 /* s */
	const val CACHE_TIME_MONS = 60 * 60 * 24 * 30 /* s */
	const val CACHE_TIME_YEAR = 60 * 60 * 24 * 365 /* s */
	...
	private fun offlineRequest(request: Request, time: String? = null) = request(request) {
		header("Cache-Control", "only-if-cached, max-stale=${time ?: DEF_CACHE_STALE}")
	}
	...
	private fun onlineChain(chain: Interceptor.Chain): Interceptor.Chain {
		...
		val timeOutConnection = request.header(HEADER_TIMEOUT_CONNECTION)?.toIntOrNull() ?: chain.connectTimeoutMillis()
		val timeOutRead = request.header(HEADER_TIMEOUT_READ)?.toIntOrNull() ?: chain.readTimeoutMillis()
		val timeOutWrite = request.header(HEADER_TIMEOUT_WRITE)?.toIntOrNull() ?: chain.writeTimeoutMillis()
		return chain
